...
function program_load_image_widget() {
	register_widget('program_Image_Widget');
}
add_action('widgets_init', 'program_load_image_widget');

/**
 * program_Image_Widget class
 **/
class program_Image_Widget extends WP_Widget {
	 ...
	 */
	function program_Image_Widget() {
		$this->loadPluginTextDomain();
		$widget_ops = array( 'classname' => 'widget_sp1_image', 'description' => __( 'Showcase a single image with a Title, URL, and a Description', 'image_widget' ) );
		$control_ops = array( 'id_base' => 'widget_sp1_image' );
		$this->WP_Widget('widget_sp1_image', __('Program Post Widget', 'image_widget'), $widget_ops, $control_ops);
	 ...
	 * @author Modern Tribe, Inc. (Peter Chester)
	 */
	function is_sp1_widget_context() {
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['avg_salary'] = $new_instance['avg_salary'];
		$instance['feat_job'] = $new_instance['feat_job'];
