<?php
/*
Plugin Name: Custom Head image Widget
...
function head_load_image_widget() {
	register_widget('Head_Image_Widget');
}
add_action('widgets_init', 'head_load_image_widget');

/**
 * Tribe_Image_Widget class
 **/
class Head_Image_Widget extends WP_Widget {
	 ...
	 */
	function Head_Image_Widget() {
		$this->loadPluginTextDomain();
		$widget_ops = array( 'classname' => 'widget_head_image', 'description' => __( 'Showcase a single image with a Title, URL, and a Description', 'image_widget' ) );
		$control_ops = array( 'id_base' => 'widget_head_image' );
		$this->WP_Widget('widget_head_image', __('Custom Head Image Widget', 'head_image_widget'), $widget_ops, $control_ops);
	...
	function admin_setup() {
		...
		elseif ( 'media-upload.php' == $pagenow || 'async-upload.php' == $pagenow ) {
			...
			add_filter( 'gettext', array( $this, 'ireplace_text_in_thickbox' ), 1, 3 );
	 ...
	 * @author Modern Tribe, Inc. (Peter Chester)
	 */
	function iis_sp_widget_context() {
	 ...
	 * @author Modern Tribe, Inc. (Peter Chester)
	 */
	function ireplace_text_in_thickbox($translated_text, $source_text, $domain) {
		if ( $this->iis_sp_widget_context() ) {
	...
	function getTemplateHierarchy($template) {
		...
		if ( $theme_file = locate_template(array('custom-image-widget/'.$template)) ) {
			$file = $theme_file;
		} else {
			$file = 'views/' . $template;
		}
