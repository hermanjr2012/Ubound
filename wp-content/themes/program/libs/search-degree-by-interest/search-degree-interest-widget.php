<?php
/*
Plugin Name: Program Image Widget
Version: 3.3.4
*/
...
function degree_interest_load_post_interest() {
	register_widget('degree_interest_post_interest');
}
add_action('widgets_init', 'degree_interest_load_post_interest');

/**
 * program_post_interest class
 **/
class degree_interest_post_interest extends WP_Widget {
	 ...
	 */
	function degree_interest_post_interest() {
		$this->loadPluginTextDomain();
		$widget_ops = array( 'classname' => 'widget_sp_interestimage', 'description' => __( 'This widget is for search degree by interest', 'post_interest' ) );
		$control_ops = array( 'id_base' => 'widget_sp_interestimage' );
		$this->WP_Widget('widget_sp_interestimage', __('Accordion Post widget for search by interest', 'post_interest'), $widget_ops, $control_ops);
	...
	function admin_setup() {
		...
		if ( 'widgets.php' == $pagenow ) {
			...
			wp_enqueue_script( 'tribe-image-widget', get_template_directory_uri(). '/libs/program-image-widget/resources/js/image-widget.js', array('thickbox'), FALSE, TRUE );
			add_action( 'admin_head-widgets.php', array( $this, 'admin_head' ) );
		}
		elseif ( 'media-upload.php' == $pagenow || 'async-upload.php' == $pagenow ) {
			wp_enqueue_script( 'tribe-image-widget-fix-uploader', get_template_directory_uri(). '/libs/program-image-widget/resources/js/image-widget-upload-fixer.js', array('jquery'), FALSE, TRUE );
	...
	function loadPluginTextDomain() {
		load_plugin_textdomain( 'post_interest', false, trailingslashit(basename(dirname(__FILE__))) . 'lang/');
	}
	 ...
	 * @author Modern Tribe, Inc. (Peter Chester)
	 */
	function is_sp_interestwidget_context() {
	...
	function replace_text_in_thickbox($translated_text, $source_text, $domain) {
		if ( $this->is_sp_interestwidget_context() ) {
			if ('Insert into Post' == $source_text) {
				return __('Insert Into Widget', 'post_interest' );
			}
		}
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['cust_url'] = $new_instance['cust_url'];
		$instance['cont_heading'] = $new_instance['cont_heading'];
